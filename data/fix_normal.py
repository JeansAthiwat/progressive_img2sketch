        ...
        for line in lines:
            ...
            if line.startswith('mtllib'):
                ...
                if os.path.exists(mtl_file_path):
                    ...
                    fix_mtl_map_kd_path(absolute_mtl_path, obj_name)  # Fix texture paths in the MTL file
                else:
                    new_lines.append(line)  # If the MTL file is not found, keep the original line
            else:
        ...
        for line in lines:
            ...
            if line.startswith('map_Kd'):
                ...
                if os.path.exists(corrected_texture_path):
                    ...
                    new_lines.append(line)  # If the texture file is not found, keep the original line
            else:
                new_lines.append(line)
...
def correct_texture_path(mtl_dir, texture_filename, obj_name):
    ...
    if len(parts) > 1:
        parts[-2] = obj_name  # Replace the second-to-last part with the correct folder name
...
def normalize_obj_model(obj_file_path):
    ...
    for line in lines:
        if line.startswith('v '):
            ...
            new_lines.append(f"v {normalized_vertex[0]} {normalized_vertex[1]} {normalized_vertex[2]}\n")
        else:
            new_lines.append(line)
...
root_directory = "/ssd/du_dataset/mvdfusion/LOD_dataset/"
